...
/**
 * Singleton manager for Dirac's temporary files.
 */
class DiracTempManagerImpl {
	...
	/**
	 * Clean up old Dirac temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Dirac temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Dirac temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Dirac temp cleanup", error)
		}
...
export const DiracTempManager = new DiracTempManagerImpl()
