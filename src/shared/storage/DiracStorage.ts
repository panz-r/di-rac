...
export interface DiracStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: DiracStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and DiracSyncStorage both satisfy this interface.
 */
 ...
 */
export abstract class DiracStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "DiracStorage"
	/**
 ...
 * Unlike DiracStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: DiracStorageChangeEvent) => void

export abstract class DiracSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of DiracStorage using a Map.
 */
export class DiracInMemoryStorage extends DiracStorage {
	/**
